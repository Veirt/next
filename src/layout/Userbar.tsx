                    ...
                    {!item.onClick ? (
                      ...
                      <div className="relative">
                        <button
                          ...
                          className={`nav-link ${item.title === 'component.navbar.notifications' && toggleNotifications ? 'text-orange-400' : 'text-white'} hover:text-orange-400 text-lg rounded tracking-wider uppercase px-0.5 hover:text-orange-400 transition ease-in-out duration-300 font-semibold`}
                        >
                          <FontAwesomeIcon icon={item.icon.name} />
                          {item.title === 'component.navbar.notifications' && notificationCount > 0 && <div className="absolute -bottom-px -right-0.5 bottom-0.5 border-2 border-gray-775 bg-blue-400 rounded-full h-3 w-3 flex items-center justify-center" />}
                        </button>
                          ...
                          <div className={`${toggleNotifications ? 'is-active' : 'is-not'} dropdown dropdown-2xl w-128 shadow-lg right-0`}>
                            <div className={'bg-gray-700 rounded-t-2xl shadow flex py-2 px-4'}>
                              <div className={'w-auto mr-auto'}>
                                <span className={'text-base  text-white uppercase font-semibold'}>{t('component.navbar.notifications')}</span>
                              </div>
                              <div className={'w-auto my-auto'}>
                            ...
                            {notificationData !== null && (
                              <div className={'h-96 bg-gray-750 rounded-b-2xl overflow-y-scroll overflow-x-hidden'}>
                                {notificationData?.length !== 0 ? notificationData?.map((row, index) => <Notification key={'notification' + (row._id || index)} {...row} />) : <div className="pt-16 text-center">You have no notifications!</div>}
                              </div>
                            )}
                          </div>
                        )}
