...
const Userbar = (props: IProps) => {
  ...
  const { sessionData, isGuest, notificationData, notificationCount, deleteNotifications, readNotifications } =
    usePlayerContext();
  ...
  const notificationToggleManual = (e: MouseEvent) => {
    if ((e.target as Element).classList.contains('notificationsWrapper')) setToggleNotifications(false);
  };
  ...
  return (
    <>
      ...
      {isLoaded && toggleNotifications && (
        <div className={'notificationsWrapper fixed z-10 top-0 right-0 left-0 bottom-0 w-screen h-screen '} />
      )}
      {isLoaded && sessionData !== null ? (
        <div
          className={`flex flex-wrap ${
            isSidebar ? 'flex-row-reverse justify-center gap-y-4 lg:gap-y-0' : 'justify-center lg:justify-end'
          }`}
        >
          {sessionData && isGuest ? (
            <>
              <div className={`w-auto my-auto`}>
                <div className="hidden lg:flex text-base text-white font-semibold tracking-wider">
                  ...
                  <div className={'w-auto my-auto pl-2'}>{sessionData.name}</div>
                </div>
              </div>
              <div className={`w-auto relative ${isSidebar ? 'pl-0 lg:pl-3' : 'pl-3'}`}>
                <Link
                  to="/auth/login"
                  className="block focus:outline-none text-base hover:bg-gray-775 rounded-lg tracking-wider uppercase py-2 px-3 hover:bg-opacity-70 transition ease-in-out duration-200 text-white font-semibold"
                >
                ...
                ((!item.isAuth && isGuest) || (!isGuest && item.isAuth)) && (
                  <div key={'userbar' + item.title} className={`w-auto my-auto ${isSidebar ? 'pl-0 lg:pl-3' : 'pl-3'}`}>
                    {!item.onClick ? (
                      <Link
                        to={item.route}
                        className={`activeClassName[text-orange-400] nav-link text-white text-lg rounded tracking-wider uppercase px-0.5 hover:text-orange-400 transition ease-in-out duration-300 font-semibold`}
                      >
                      ...
                      <div className="relative">
                        <button
                          ...
                          className={`nav-link ${
                            item.title === 'component.navbar.notifications' && toggleNotifications
                              ? 'text-orange-400'
                              : 'text-white'
                          } hover:text-orange-400 text-lg rounded tracking-wider uppercase px-0.5 hover:text-orange-400 transition ease-in-out duration-300 font-semibold`}
                        >
                          <FontAwesomeIcon icon={item.icon.name} />
                        ...
                        {item.title === 'component.navbar.notifications' && (
                          <div
                            className={`${
                              toggleNotifications ? 'is-active' : 'is-not'
                            } dropdown dropdown-2xl w-128 shadow-lg right-0`}
                          >
                            <div className={'bg-gray-700 rounded-t-2xl shadow flex py-2 px-4'}>
                              ...
                              <div className={'w-auto my-auto'}>
                                <button
                                  className={
                                    'focus:outline-none text-orange-400 hover:opacity-70 transition ease-in-out duration-300'
                                  }
                                  type={'button'}
                              ...
                              <div className={'h-96 bg-gray-750 rounded-b-2xl overflow-y-scroll overflow-x-hidden'}>
                                {notificationData?.length !== 0 ? (
                                  notificationData?.map((row, index) => (
                                    <Notification key={'notification' + (row._id || index)} {...row} />
                                  ))
                                ) : (
          ...
          {!isGuest && (
            <Link
              to={`/profile/${sessionData.name}-${sessionData.discriminator}`}
              className={`flex w-auto my-auto ${isSidebar ? 'pl-0 lg:pl-3' : 'pl-3'}`}
            >
              <div
                className={
                  'block w-12 my-auto p-0.5 ml-3 border-2 border-gray-500 border-opacity-50 hover:border-orange-400 hover:border-opacity-50 transition ease-in-out duration-200 rounded-full'
                }
              >
