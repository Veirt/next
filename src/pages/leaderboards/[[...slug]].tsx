...
const Leaderboards = (props: IProps) => {
  ...
  const getTexts = useCallback(() => {
    axios.get(`${Config.apiUrl}/leaderboards/matches?textId=${parseInt(filter, 10) || 2}&flagId=0&startNum=${skip}&limit=50`, { cancelToken: axiosCancelSource.current?.token }).then((response) => {
      if (!response.data.error) {
        setData([...response.data.data]);
        setDataPage(response.data.isNextPage);
        setLoaded(true);
    ...
    if (sessionData?.playerId) {
      axios.get(`${Config.apiUrl}/leaderboards/matches?textId=${parseInt(filter, 10) || 2}&flagId=0&playerId=${sessionData?.playerId}`, { cancelToken: axiosCancelSource.current?.token }).then((response) => {
        if (!response.data.error) setPlayerData([...response.data.data]);
        else toast.error('Unable to pull rank data');
      });
    }
  ...
  const getTextSearch = (v: string) => {
    axios.get(`${Config.apiUrl}/texts/list?search=${v}&limit=5`, { cancelToken: axiosCancelSource.current?.token }).then((response) => {
                    ...
                    <div className={`dropdown dropdown-gap-small overflow-hidden w-full ${dropdown && dropdown === 2 ? 'is-active' : 'is-not'}`}>
                      ...
                      {textData.map((item) => (
                        <Link key={item.textId} to={`/leaderboards/texts/${item.textId}`} className={`item ${item.textId === parseInt(filter, 10) && 'is-active'}`}>
                          {item.textId}. {item.content.split(' ').map((item, index) => index <= 16 && `${item}${index !== 16 ? ' ' : ''}`)}
