              ...
              <div className={'flex flex-wrap gap-x-2 text-lg text-white uppercase'}>
                <div className={'relative'}>
                  ...
                  <div className={`dropdown w-40 ${dropdown === 1 ? 'is-active' : 'is-not'}`}>
                    {types.map((item) => (
                      <button
                        key={`filterType_${item.id}`}
                        type={'button'}
                        className={`item ${filterType === item.id ? 'is-active' : ''}`}
                        onClick={() => {
                ...
                <div className={'relative'}>
                  ...
                  <div className={`dropdown w-44 ${dropdown === 2 ? 'is-active' : 'is-not'}`}>
                    {locales.map((item) => (
                      <button
                        key={`filterType_${item.key}`}
                        type={'button'}
                        className={`item ${filterLocale === item.key ? 'is-active' : ''}`}
                        onClick={() => {
                          setFilterLocale(item.key);
                          setDropdown(0);
                        }}
                      >
                        {item.name}
