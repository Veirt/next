...
const Login = () => {
    ...
    const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
        ...
        axios.post(`${Config.authUrl}/login`, { emailAddress, password, _csrf }, { cancelToken: axiosCancelSource.current?.token, withCredentials: true })
            .then((response) => {
                        ...
                        <form method="post" onSubmit={handleSubmit}>
                            ...
                            <div className="grid grid-cols-5 gap-y-2">
                                <div className="col-span-full text-center xs:text-left xs:col-span-3 my-auto">
                                    <Link to="/auth/forgot" className="hover:opacity-70 transition ease-in-out duration-300">Forgot your password?</Link>
                                </div>
                                <div className="col-span-full xs:col-span-2">
                                    <button type="submit" className="flex mx-auto xs:mr-0 xs:ml-auto button small lightgray">Login</button>
                                </div>
                            </div>
                        </form>
                    </div>
                    <Link to="/auth/signup" className="text-lg block mt-4 text-center font-semibold hover:opacity-70 transition ease-in-out duration-300">Don't have an account?</Link>
                </div>
            </div>
        </Base>
    )
