...
const ProfileAchievements: FC<IProps> = (props) => {
    ...
    return (
        <div>
            <h2 className={"headingBox"}>{t('statistics.achievements')}</h2>
            {sections.map((section) => (
                <div key={section.string} className={"mb-8 mt-4"}>
                    <h3 className={"text-white uppercase font-semibold pb-3"}>{t(section.string)}</h3>
                    <div className={"grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4"}>
                        {achievementsData.map((item) => item.type === section.key && (
                            <div className={`${item.value ? 'opacity-100' : 'opacity-30'}`}>
                                <div className={"flex text-white"}>
                                    ...
                                    <div className={"w-full my-auto bg-gray-750 rounded-xl pl-12 py-2 shadow-md pl-4"}>
                                        <div className={"uppercase text-xl font-semibold"}>{item.title}</div>
                                        {item.value ? (
