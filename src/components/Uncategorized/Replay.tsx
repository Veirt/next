...
const parseReplay = (logString: string) => {
  ...
  listOfEvents.map((item) => {
    ...
    if (useLastEvent && !useLastEvent?.input.endsWith(' ')) {
      ...
      if (triggerFakeEvent) parsedEvents.push({ input: '', delay: 0 });
    }
  ...
  return (
    <div>
      ...
      <button
        ...
        className="text-sm font-semibold text-orange-400 mt-2 hover:opacity-70 transition ease-in-out duration-300"
      >
