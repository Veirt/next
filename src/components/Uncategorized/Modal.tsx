...
const Modal = (props: IProps) => {
  ...
  return (
    <div
      ...
      className={`modal-bg fixed inset-0 w-full h-screen bg-black bg-opacity-50 transition-all ease-in-out duration-300 ${
        props.isOpened ? 'opacity-100 visible' : 'opacity-0 pointer-events-none invisible'
      }`}
    >
      <div
        className={`modal-bg flex h-screen transform transition-all ease-in-out duration-300 ${
          props.isOpened ? 'translate-y-0' : 'translate-y-1'
        }`}
      >
        <div className={`modal-bg ${sizeCSS} m-auto relative`}>
          <div className={`${props.className || 'bg-gray-775 p-8'} w-full rounded-2xl shadow-lg relative`}>
            <button
              type="button"
              className="focus:outline-none text-2xl text-white text-opacity-70 hover:text-opacity-40 absolute top-2 right-2.5 transition"
            >
