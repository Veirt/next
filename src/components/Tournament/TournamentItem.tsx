...
const TournamentItem = (props: IProps) => {
  ...
  if (useSplit.length > 96) useValue = useValue?.substring(0, 80) + '...' || '';
      ...
      <div className={'w-full mt-auto pt-5'}>
        <div className={'flex space-x-2 text-sm'}>
          ...
          <div className={'uppercase px-3 py-1.5 bg-gray-825 rounded-lg'}>
            ...
            {props.status === 0 && `${t('page.tournaments.starts')} ${moment.unix(props.startTime).fromNow()}`}
            {props.status === 1 && `${t('page.tournaments.ends')} ${moment.unix(props.endTime).fromNow()}`}
