...
const LeaderboardPlayerTournament: FC<IProps> = (props) => {
  ...
  for (i = 0; i < dataLength; i++) {
    const item = data[i];
    if (qualifierSort !== 'avgWPM' || (qualifierSort === 'avgWPM' && item && item.matchesTotal >= recentWPMCutoff)) isQualified.push(data[i]);
    ...
    <div>
      ...
      {isQualified.map((item, key) => (
        <div key={item?.player[0]?.playerId} className="flex leaderboards--row">
          <div className={`hidden md:block my-auto w-10 text-center font-bold py-3 ${qualifier === 1 && (qualifierCutoff >= key + 1 ? 'bg-green-400 bg-opacity-30' : 'bg-yellow-400 bg-opacity-30')}`}>
