...
const LeaderboardPlayerTournament:FC<IProps> = (props) => {
    ...
    for (i = 0; i < dataLength; i++) {
        ...
        else if (qualifierSort === 'avgWPM' && (item && item.matchesTotal < recentWPMCutoff))
            isNotQualified.push(data[i]);
    }
                ...
                <div key={item?.player[0]?.playerId} className="flex leaderboards--row">
                    ...
                    {prizingJSON && prizingJSON[key] && (
                        <div className={`hidden md:block w-32 my-auto ${status === 2 ? 'opacity-100' : 'opacity-50'}`}>
                            {prizingJSON[key].type === 'currency' ? <FontAwesomeIcon icon={faCoins} className={"text-yellow-400 text-base"} /> : <span className={"text-white uppercase text-xs font-semibold"}>{prizingJSON[key].type}</span>}
                            <span className={"text-white uppercase text-base font-semibold pl-2"}>{prizingJSON[key].type === 'currency' ? prizingJSON[key].value.toLocaleString() : ''}</span>
                        </div>
                    )}
                    {/* @ts-ignore */}
                    <div className="hidden md:block w-28 my-auto">{isWPM.includes(qualifierSort) ? (useCPM === '1' ? `${(item[qualifierSort] * 5).toFixed(2)} CPM` : `${item[qualifierSort].toFixed(2)} WPM`) : `${(item[qualifierSort] ?? 0).toLocaleString()} matches`}</div>
                    <div className={"w-8 md:hidden"} />
                </div>
            ))}
            ...
            {qualifierSort === "avgWPM" && (
                isNotQualified.map((item) =>  (
                    <div key={item?.player[0]?.playerId} className="flex leaderboards--row opacity-70">
                        ...
                        <div className="hidden md:block w-28 my-auto">{useCPM === '1' ? `${((item?.avgWPM || 0) * 5).toFixed(2)} CPM` : `${(item?.avgWPM || 0).toFixed(2)} WPM`}</div>
                        <div className={"w-8 md:hidden"} />
                    </div>
                ))
            )}
