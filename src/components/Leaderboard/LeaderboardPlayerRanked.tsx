    ...
    <div>
          ...
          item.rating && (
            <div key={`${key + skip}${item.rating}${item.player[0] ? item.player[0].playerId : ''}`} className="flex leaderboards--row">
              ...
              <div className={'w-12 lg:w-10 my-auto'}>
                <img className={'w-7'} src={`/ranks/${item.Rank ? `${item.Rank.Rank.replaceAll(' ', '').toLowerCase()}` : 'unrated'}.svg`} alt={'Unrated'} />
              </div>
              <div className={'hidden md:block w-32 my-auto font-semibold'}>
                {item.Rank.Rank}
                <div className="opacity-50 text-xs -mt-1">{item.rating} SR</div>
              </div>
              <div className={'hidden md:block w-16 my-auto text-center font-semibold'}>{item.matchesWon.toLocaleString()}</div>
              <div className={'hidden md:block w-16 my-auto text-center font-semibold'}>{(item.matchesWon + item.matchesLost + item.matchesQuit).toLocaleString()}</div>
              <div className={'w-8 md:hidden'} />
            </div>
          ),
      )}
      ...
      {playerData.map(
        (item, key) =>
          item.Rank &&
          item.rating && (
            <div key={`data${key}${item.rating}${item.player[0] ? item.player[0].playerId : ''}`} className="flex leaderboards--row leaderboards--highlight">
