...
const LeaderboardPlayerStatistic: FC<IProps> = (props) => {
  ...
  const updateValue = (fieldName: string, value: number) => {
    if (fieldName === 'experience') return (value / 1000).toFixed(1) + 'K EXP';
      ...
      return (
        <>
          {(value || 0).toLocaleString()} <span className="bg-gray-600 px-1 py-0 rounded-full">CR</span>
        </>
      );
    else return value;
  };
  ...
  const updateColumn = (fieldName: string) => {
    if (fieldName === 'experience') return 'EXP';
    else if (fieldName === 'playtime') return 'Time';
    else if (fieldName === 'highestWPM') return 'WPM';
    else if (fieldName === 'matchesWon') return 'Wins';
    else if (fieldName === 'count')
      // Challenges
      return 'Challenges';
    else if (fieldName === 'cr') return 'Score';
