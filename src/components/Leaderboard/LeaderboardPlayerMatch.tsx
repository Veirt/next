  ...
  return (
    <div>
      ...
      {data.map((item, key) => (
        <div key={item.player && item.player[0] ? item.player[0].playerId : key} className="flex leaderboards--row">
          <div className="hidden md:block w-10 text-center font-bold my-auto">
            {!disableTrophy ? <FormatIndex index={key + skip + 1} /> : (key + skip + 1).toLocaleString()}
          </div>
          ...
          <div className="w-32 my-auto">
            {useCPM === '1' ? `${((item?.wpm || 0) * 5).toFixed(2)} CPM` : `${(item?.wpm || 0).toFixed(2)} WPM`}
          </div>
      ...
      {playerData.map((item, key) => (
        <div
          key={'data' + String(item.player && item.player[0] ? item.player[0].playerId : key)}
