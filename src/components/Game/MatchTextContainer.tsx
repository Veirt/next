...
const MatchTextContainer = (props: IProps) => {
  ...
  useEffect(() => {
    ...
    if (!refreshFPS.current) refreshFPS.current = setInterval(onRefreshFPS, useFPS);

    if (inputElement.current) inputElement.current?.addEventListener('blur', onFocus);
  ...
  useEffect(() => {
    if (isSuddenDeath && typoStreak >= 1) setDisable(true);
  ...
  useEffect(() => {
    if (!disable) inputElement.current?.focus();
  }, [disable]);
  ...
  const onChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (inputLength === 0) {
      ...
      if (inputLength > currentInputLength) {
        ...
        let i: number,
          isTypoStreak: boolean = false;
        for (i = 0; i < inputLength; i++) {
          // console.log(inputValue.charAt(i), ' vs ', quote.charAt(wordIndex + i));

          if (inputValue.charAt(i) !== quote.charAt(wordIndex + i)) isTypoStreak = true;

          if (isTypoStreak) currentTypoStreak++;
          else currentCorrectStreak++;
        }
  ...
  return (
    <>
      <div className="flex flex-wrap match--text w-4 opacity-0 absolute left-0 top-0 pointer-events-none">
        <div ref={letterElement} className={`match--letter ${upscaleMatchCSS || ''}`}>
          A
        </div>
      </div>
      <div className={`${!isReplay && matchContainerTransparent === '1' ? 'match--container-transparent' : 'match--container'}`}>
        <div ref={containerElement} className={`match--text ${matchTextTypeCSS || ''} ${upscaleMatchCSS || ''}  relative pointer-events-none overflow-y-hidden`}>
          ...
          <span ref={currentElement} className={`match--letter ${typoStreak ? colorBlindCSS : 'text-gray-400'}`}>
            {current}
          </span>
