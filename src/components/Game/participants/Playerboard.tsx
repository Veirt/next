      ...
      {participantsData.map(
        (item, index) =>
          ...
          item.teamId !== 0 && (
            <div
              key={item.playerId}
              className={`overflow-hidden relative shadow-lg flex flex-wrap text-gray-500 bg-gray-750 w-full pt-1 font-semibold text-xs rounded-xl`}
            >
              ...
              <div
                className={`w-auto h-full mt-1 border-l-4 rounded-b-xl ${
                  borderColors && isSpectator === 1
                    ? borderColors[index]
                      ? borderColors[index]
                      : borderColors[1]
                    : 'border-orange-400'
              ...
              <div className={'hidden lg:block w-60 my-auto'}>
                {isSpectator ? (
                  <>
                    {item.Progress !== 100 && quoteString !== 'KEYMASH_GAMEMODE_ROUND_END' ? (
                      <div className={'truncate px-3 h-6 pt-1 rounded bg-gray-700 font-semibold text-white'}>
                        ...
                        <span className={'border-b border-orange-400 text-orange-400'}>{item.currentInput || ''}</span>
                      </div>
                    ) : (
                      (item.Progress === 100 || quoteString === 'KEYMASH_GAMEMODE_ROUND_END') && (
                        <div className={'text-gray-500 uppercase text-base font-semibold tracking-wide'}>
                          {t('other.finished')}
                        </div>
                      )
                    )}
                  ...
                  <>
                    {item.Progress !== 100 && quoteString !== 'KEYMASH_GAMEMODE_ROUND_END' ? (
                      <div className={'truncate font-semibold text-white text-base text-center'}>
                        {item.currentWord || firstWord}
                      </div>
                    ) : (
                      (item.Progress === 100 || quoteString === 'KEYMASH_GAMEMODE_ROUND_END') && (
                        <div className={'text-gray-500 uppercase text-base font-semibold tracking-wide'}>
