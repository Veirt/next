          ...
          item.teamId !== 0 && (
            <div key={item.playerId} className={`overflow-hidden relative shadow-lg flex flex-wrap text-gray-500 bg-gray-750 w-full pt-1 font-semibold text-xs rounded-xl`}>
              ...
              <div className={`w-full sm:w-1/3 md:w-64`}>
                <PlayerCard className={`pl-4 pr-2 py-2.5`} cardBorder={item.cardBorder} cardImage={item.cardImage} name={item.name} discriminator={item.discriminator} avatarSrc={item.avatarSrc} verified={item.verified} patreon={item.patreon} staff={item.staff} useTransparent>
                  <div>
