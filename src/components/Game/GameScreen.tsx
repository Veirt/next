...
import { SocketGameEndData, SocketMatchData, SocketMatchPlayerData } from '../../types.client.socket';
...
const GameScreen = (props: IProps) => {
  ...
  useEffect(() => {
    ...
    socket.onConnectNotSaved(() => {
      setGameNotice('reconnectFailed');
    ...
    socket.on('updatePlayers', (data: SocketMatchPlayerData[]) => {
      if (!data) toast.error('updatePlayers unable to fetch players!');
      else {
    ...
    socket.on('updateWPM', (data: SocketMatchPlayerData) => {
      if (data.spectatorOnly && !spectator.current) return;
  ...
  const handleKeydown = (e: KeyboardEvent) => {
    ...
    if (!endMatchData && gameCountdown >= 1 && e.key === 'Backspace') e.preventDefault();
  };
          ...
          <>
            ...
            >
              <Player
                ...
                translation={t('page.match.statistics_unsaved')}
                firstWord={firstWord}
                ...
                endMatch={endMatchData !== null}
                endMatchData={endMatchData as SocketGameEndData}
                leaveUrl={leaveUrl}
                matchData={matchData}
                participantsData={participantsData}
                noticeString={noticeString}
                restartUrl={restartUrl}
                roundsTotal={gameRoundsTotal}
              />
            </div>
          </>
        ) : (
