...
const MatchCountdown: FC<MatchCountdownProps> = (props) => {
  ...
  const playCountdownAudio = useCallback(() => {
    ...
    if (countdown !== 99) {
      if (countdown >= 1 && getBeep && countdownBeep === '1') getBeep?.play();

      if (countdown === 0 && getStart && countdownBeep === '1') getStart?.play();
    }
  }, [countdown, countdownBeep]);

  useEffect(() => {
    const countdownTimer = document.getElementById('countdownTimer') as HTMLElement;
    if (props.roundEnd && countdownTimer) countdownTimer.style.opacity = '1';
  }, [props.roundEnd]);
          ...
          <div className="w-full flex h-screen bg-black bg-opacity-70">
            <div className="m-auto">
              <div className={'max-w-screen-sm mx-auto'}>
                ...
                {isSpectator && roundEnd && <div className={'text-center text-2xl pb-8 uppercase text-white font-bold tracking-wider'}>Round Completed!</div>}
                <div className="w-32 mx-auto rounded bg-gray-775 bg-opacity-75 shadow text-center py-3">
                  <div style={{ fontSize: '4em' }} className={`font-bold pb-1 ${countdownClass}`}>
                    {countdown >= 15 ? <FontAwesomeIcon icon={faCircleNotch} className="text-blue-400" spin /> : <span>{renderCountdown > 0 ? renderCountdown : <span>GO</span>}</span>}
                  </div>
                </div>
                {!isSpectator && roundEnd && <div className={'text-lg pt-8 uppercase text-white font-semibold tracking-wider'}>The next round will be starting soon...</div>}
              </div>
            </div>
          </div>
        </div>
