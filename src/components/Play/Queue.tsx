...
import { faList, faPlusSquare, faRandom } from '@fortawesome/free-solid-svg-icons';
...
const Queue = (props: IProps) => {
  ...
  const getLevel = useCallback(() => {
    if (sessionData) {
      axios
        .get(`${Config.apiUrl}/player/ranked`, { withCredentials: true, cancelToken: axiosCancelSource.current?.token })
        .then((response) => {
  ...
  const pingCreateMatch = useCallback(
    async (textType: string) => {
      setLoading(1);
  ...
  useEffect(() => {
    if (mode) pingCreateMatch(mode).then();
        ...
        {tabs.map((tab, index) => (
          <div key={index} className={`relative col-span-full lg:col-span-1 px-8 text-center lg:text-left`}>
            ...
            <div className="relative">
              ...
              <p className="pt-2 pb-6 block relative z-10 text-gray-300 truncate">{t(tab?.description)}</p>
              {/* Buttons */}
