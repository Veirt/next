...
const LobbiesRow: FC<LobbyData> = (props) => {
  return props.cardImage && props.cardBorder && props.name && props.avatarSrc ? (
    <Link
      to={`/custom/${props.invite}`}
      className="flex bg-gray-750 hover:bg-gray-775 shadow-md mb-2 rounded transition ease-in-out duration-200"
    >
      <PlayerCard
        className="pointer-events-none w-52 px-4 py-2 rounded-l"
        useTransparent
        useSmall
