...
const SettingsFrame = (props: IProps) => {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        if (!response.data.error) {
          response.data.map((item: { itemId: string }) =>
  ...
  const handleApiKey = () => {
    ...
    axios
      .post(`${Config.apiUrl}/player/apiKey`, formData, {
        cancelToken: axiosCancelSource.current?.token,
        withCredentials: true,
      })
      .then((response) => {
  ...
  const handleSave = () => {
    ...
    axios
      .post(`${Config.apiUrl}/player/settings`, formData, {
        cancelToken: axiosCancelSource.current?.token,
        withCredentials: true,
      })
      .then((response) => {
  ...
  const options = [
    {
      form: [
        ...
        },
        {
          ...
          options: [
            {
              title: 'page.profile.keyboard.layout',
              name: 'keyboardLayout',
            ...
            {
              title: 'page.profile.keyboard.brand',
              name: 'keyboardBrand',
              value: keyboardBrand,
              onChange: (v: string) => {
                setUnsaved(true);
            ...
            {
              title: 'page.profile.keyboard.model',
              name: 'keyboardModel',
              value: keyboardModel,
              onChange: (v: string) => {
                setUnsaved(true);
      ...
      form: [
        {
          ...
          options: [
            {
              title: 'page.profile.locale',
              name: 'locale',
              value: config.locale,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, locale: String(v) });
              },
              type: 'selectLocale',
              localeList: locales,
            },
            {
              title: 'page.profile.streamerMode',
              name: 'streamerMode',
              value: config.streamerMode,
              onChange: (v: number) => {
                setUnsaved(true);
                setConfig({ ...config, streamerMode: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
            {
              title: 'page.profile.wpm',
              name: 'useCPM',
              value: config.useCPM,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, useCPM: String(v) as '0' | '1' });
              },
              type: 'configWPM',
            },
            {
              title: 'page.profile.networkStrength',
              name: 'networkStrength',
              value: config.networkStrength,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, networkStrength: String(v) as '0' | '1' | '2' });
              },
        ...
        },
        {
          ...
          options: [
            {
              title: 'page.profile.matchTextSize',
              name: 'upscaleMatch',
              value: config.upscaleMatch,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, upscaleMatch: String(v) as '0' | '1' });
              },
              type: 'configScale',
            },
            {
              title: 'page.profile.matchTextType',
              name: 'matchTextType',
              value: config.matchTextType,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, matchTextType: String(v) as '0' | '1' });
              },
              type: 'configMatchText',
            },
            {
              title: 'page.profile.matchTextScroll',
              name: 'matchTextScroll',
              value: config.matchTextScroll,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, matchTextScroll: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
            {
              title: 'page.profile.upscaleMatchContainer',
              name: 'upscaleMatchContainer',
              value: config.upscaleMatchContainer,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, upscaleMatchContainer: String(v) as '0' | '1' });
              },
              type: 'configScale',
            },
            {
              title: 'page.profile.matchContainerTransparent',
              name: 'matchContainerTransparent',
              value: config.matchContainerTransparent,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, matchContainerTransparent: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
            {
              title: 'page.profile.gameplayParticipantStyle',
              name: 'gameplayParticipantStyle',
              value: config.gameplayParticipantStyle,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, gameplayParticipantStyle: String(v) as '0' | '1' });
              },
              type: 'configPlayercardList',
            },
            {
              title: 'page.profile.inputbox',
              name: 'hideInputBox',
              value: config.hideInputBox,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, hideInputBox: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
            {
              title: 'page.profile.smoothCaret',
              name: 'smoothCaret',
              value: config.smoothCaret,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, smoothCaret: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
            {
              title: 'page.profile.smoothCaretSpeed',
              name: 'smoothCaretSpeed',
              value: config.smoothCaretSpeed,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, smoothCaretSpeed: String(v) as '0' | '1' });
              },
              type: 'selectSmoothCaretSpeed',
              smoothCaretList,
            },
            {
              title: 'page.profile.hideWPM',
              name: 'hideWPM',
              value: config.hideWPM,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, hideWPM: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
            {
              title: 'page.profile.focusMode',
              name: 'focusMode',
              value: config.focusMode,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, focusMode: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
            {
              title: 'page.profile.performanceMode',
              name: 'performanceMode',
              value: config.performanceMode,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, performanceMode: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
            {
              title: 'page.profile.colorBlindMode',
              name: 'colorBlindMode',
              value: config.colorBlindMode,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, colorBlindMode: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
            {
            ...
            },
            {
              ...
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, adsGameplay: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
          ],
        },
        {
          ...
          options: [
            {
              title: 'page.profile.countdownBeep',
              name: 'countdownBeep',
              value: config.countdownBeep,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, countdownBeep: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
            {
              title: 'page.profile.matchFinishBeep',
              name: 'matchFinishBeep',
              value: config.matchFinishBeep,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, matchFinishBeep: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
            {
              title: 'page.profile.customChatBeep',
              name: 'customChatBeep',
              value: config.customChatBeep,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, customChatBeep: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
            {
              title: 'page.profile.rankUpSound',
              name: 'rankUpSound',
              value: config.rankUpSound,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, rankUpSound: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
            {
              title: 'page.profile.rankDownSound',
              name: 'rankDownSound',
              value: config.rankDownSound,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, rankDownSound: String(v) as '0' | '1' });
              },
              type: 'configBoolean',
            },
          ],
        },
        {
          ...
          options: [
            {
              title: 'page.profile.shortcutHome',
              name: 'shortcutHome',
              value: config.shortcutHome,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, shortcutHome: String(v) as '0' | '1' });
              },
              type: 'configShortcut',
            },
            {
              title: 'page.profile.shortcutPlayQuotes',
              name: 'shortcutPlayQuotes',
              value: config.shortcutPlayQuotes,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, shortcutPlayQuotes: String(v) as '0' | '1' });
              },
              type: 'configShortcut',
            },
            {
              title: 'page.profile.shortcutGameRedo',
              name: 'shortcutGameRedo',
              value: config.shortcutGameRedo,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, shortcutGameRedo: String(v) as '0' | '1' });
              },
              type: 'configShortcut',
            },
            {
              title: 'page.profile.shortcutPlayRandom',
              name: 'shortcutPlayRandom',
              value: config.shortcutPlayRandom,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, shortcutPlayRandom: String(v) as '0' | '1' });
              },
              type: 'configShortcut',
            },
            {
              title: 'page.profile.shortcutExit',
              name: 'shortcutExit',
              value: config.shortcutExit,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, shortcutExit: String(v) as '0' | '1' });
              },
              type: 'configShortcut',
            },
            {
              title: 'page.profile.shortcutPlayDictionary',
              name: 'shortcutPlayDictionary',
              value: config.shortcutPlayDictionary,
              onChange: (v: string) => {
                setUnsaved(true);
                setConfig({ ...config, shortcutPlayDictionary: String(v) as '0' | '1' });
              },
              type: 'configShortcut',
            },
          ],
              ...
              <div className={'relative p-5 overflow-y-scroll'} style={{ height: '60vh' }}>
                ...
                {tab === 0 && (
                  <div className={'mb-6'}>
                    ...
                    {toggleSecret ? (
                      <div className={'mt-4'}>
                        <div className={'inline my-2 rounded-lg bg-gray-825 px-4 py-1.5 text-white'}>{apiKey}</div>

                        <div className={'mt-3'}>
                          <button type={'button'} onClick={handleApiKey} className={'button small orange'}>
                            {apiKey ? 'Generate New Secret' : 'Generate Secret'}
                          </button>
                        </div>
                      </div>
                    ) : (
                ...
                {tab === 2 && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {playercards.map(
                      (item) =>
                        inventory.includes(item.file) && (
                          <label key={item.file}>
                            <input
                              type={'radio'}
                              className={'form-control-radio-div'}
                              name={'cardImage'}
                              value={item.file}
                              onChange={(e) => {
                                setUnsaved(true);
